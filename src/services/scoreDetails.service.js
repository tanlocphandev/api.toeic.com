...
class ScoreDetailsService {
    static async create({ scoreName, scores = [] }) {
        ...
        try {
            ...
            const [foundScoreActive, foundScoreByNameExist] = await Promise.all([
                Transaction.findOne({
            ...
            if (foundScoreByNameExist) {
                throw new ConflictRequestError(
                    `Bảng điểm có tên ${scoreName} đã tồn tại`,
                    undefined,
                    {
                        scoreName: `Bảng điểm có tên ${scoreName} đã tồn tại`,
                    }
                );
            }
